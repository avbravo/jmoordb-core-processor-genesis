...
// </editor-fold>
    // <editor-fold defaultstate="collapsed" desc="IdData readIdAnnotationOfViewEntityFile(Element element, String fileToRead, IdData idData)">
     ...
     * @return
     */
    public static IdData readIdAnnotationOfViewEntityFile(Element element, String fileToRead, IdData idData) {
        IdData result = idData;
        try {
            fileToRead = pathOfFileInProject(element, fileToRead) + fileToRead;
            Path path = Paths.get(fileToRead);
...
// </editor-fold>
    ...
    public static Integer procesarId(Stream<String> lines, Stream<String> linesWork, IdData idData) {
        ...
        try {
            ...
            endId = Boolean.FALSE;
            countLines = 0;
            idData.setGenerationType(GenerationType.NONE);
            idData.setValue("");
            ...
            lines.forEach(s -> {
                ...
                if (!endId) {
                    ...
                    if (s.contains("GenerationType.NONE")) {
                        idData.setGenerationType(GenerationType.NONE);
                    } else {
                        if (s.contains("GenerationType.AUTO")) {
                            idData.setGenerationType(GenerationType.AUTO);
                        } else {
                            if (s.contains("GenerationType.OBJECTID")) {
                                idData.setGenerationType(GenerationType.OBJECTID);
                            }else{
                                if (s.contains("GenerationType.UUID")) {
                                idData.setGenerationType(GenerationType.UUID);
                            }
                            }
                        }
                    }
                    ...
                    if (s.contains("String")) {
                        ...
                        if (s.contains("Long")) {
                            ...
                            if (s.contains("ObjectId")) {
                                idData.setFieldType("ObjectId");
                                s = s.replace("ObjectId", "");
                            } else {
                                if (s.contains("UUID")) {
                                    idData.setFieldType("UUID");
                                    s = s.replace("UUID", "");
                                }
                            }
                        }
