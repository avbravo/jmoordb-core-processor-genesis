...
// <editor-fold defaultstate="collapsed" desc="analizer(ExecutableElement executableElement,TypeMirror typeEntity,RepositoryMethod repositoryMethod)">
    ...
    public static Boolean analizer(Find findAnnotation, Element element, ExecutableElement executableElement, TypeMirror typeEntity, RepositoryMethod repositoryMethod) {
        try {
...
//Almacena el where
            if (!nameOfMethod.startsWith("findAll") && !nameOfMethod.startsWith("findBy")) {
                message = nameOfMethod + "() should start with findBy or findAll";
                return Boolean.FALSE;
            }
             ...
             * findBy
             */
            if (nameOfMethod.startsWith("findBy")) {
                text = text.replace("findBy", "").trim();
                ...
                if (nameOfMethod.startsWith("findAll")) {
                    text = text.replace("findAll", "").trim();

                    text = NameOfMethodAnalizerUtil.simplifyText(text);

                    if (!processFindAll(text, havePaginationParameters, haveSortedParameters, parameters, nameOfMethod)) {
                    ...
                    if (fieldFound.isPresent()) {
                        ...
                        message = nameOfMethod + "() method name must only contain Paginaton and Sorted parameters.";
                        return Boolean.FALSE;
