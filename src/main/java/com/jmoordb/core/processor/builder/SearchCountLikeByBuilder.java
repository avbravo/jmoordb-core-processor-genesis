...
public class SearchCountLikeByBuilder {
     ...
     * @return
     */
    public static StringBuilder searchCountLikeBy(RepositoryData repositoryData, RepositoryMethod repositoryMethod) {
...
//            
            ...
            if (repositoryMethod.getCaseSensitive().equals(CaseSensitive.YES)) {
                switch (repositoryMethod.getLikeByType()) {
                    case FROMTHESTART:
                        filter = "\tDocument docX =  new Document(\"" + field + "\", new Document(\"$regex\", \"^\"+" + parametro + "));\n";
                        break;
                    case FROMTHEEND:
                        filter = "\tDocument docX =  new Document(\"" + field + "\", new Document(\"$regex\"," + parametro + "+ \"$\"));\n";
                        break;
                ...
                switch (repositoryMethod.getLikeByType()) {
                    case FROMTHESTART:
                        filter = "\tDocument docX =  new Document(\"" + field + "\", new Document(\"$regex\", \"^\"+" + parametro + ").append(\"$options\", \"i\"));\n";
                        break;
                    case FROMTHEEND:
                        filter = "\tDocument docX =  new Document(\"" + field + "\", new Document(\"$regex\", " + parametro + "+ \"$\" ).append(\"$options\", \"i\"));\n";
                        break;
